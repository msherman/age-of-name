terraform {
  backend "s3" {
    bucket = "ms-age-of-name-infra-two"
    region = "us-east-1"
    key = "terraform.state"
  }
}
...
module "react_code_build" {
  source              = "./codebuild"
  bucket_name         = aws_s3_bucket.age-of-name.bucket
  s3_arn              = aws_s3_bucket.age-of-name.arn
...
module "react_code_pipeline" {
  ...
  website_bucket         = aws_s3_bucket.age-of-name.bucket
  website_bucket_arn     = aws_s3_bucket.age-of-name.arn
